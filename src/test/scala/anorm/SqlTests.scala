...
class SqlTests extends UnitTestCase with ShouldMatchersForJUnit {
 ...
 implicit def statementInOut[A](implicit c:ColumnTo[A], ts:ToStatement[A]):(ColumnTo[A],ToStatement[A]) = (c,ts)
  ...
  @Test def useSomeMagicSqlCompileTime1{
     ...
     play.db.DB.execute("DROP TABLE IF EXISTS AAA")
     play.db.DB.execute("""CREATE TABLE AAA
                           (Id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                            Comment char(60) NOT NULL) """)
    val aaa = AAA.create(AAA(NotAssigned,"comment"))

    (aaa) should be (AAA(Id(1),"comment"))
    SQL("select * from AAA ").as(AAA) should be (AAA(Id(1),"comment"))
    AAA.update(aaa.copy(comment="comment1"))
    SQL("select * from AAA ").as(AAA) should be (AAA(Id(1),"comment1"))
