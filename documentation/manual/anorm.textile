...
p.. In the following example we will count the number of Country in the database. So the resultSet will be a single row with a single column:
...
p.. Note that since @collect(…)@ ignore the cases where the partial function isn't defined, it allow your code to safely ignore rows that you don't expect.
...
p.. Finally, let's complicate our example to separate the official language and the other ones:
...
p.. If you try this on the **world** sample database, you will get:
...
p(note). The @create@ method performs an INSERT into the database and try to retrieve the primary key from the ResultSet statement. This is useful for an auto_increment primary key field. However, if your application sets the primary key value and not the database, you must use the @insert@ method.
